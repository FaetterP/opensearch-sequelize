...
describe(".queryGet | auth", () => {
  test("success | correct auth", async () => {
    ...
    const result = await TestModel.queryGet(url, data);
  ...
  test("error | incorrect auth", async () => {
    ...
    await expect(TestModel.queryGet(url, data)).rejects.toThrow(
      "Request failed with status code 401"
    );
    await expect(TestModel.queryGet(url, data)).rejects.toThrow(AxiosError);
  });
});

describe(".queryGet | success", () => {
