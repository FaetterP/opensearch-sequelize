...
describe(".queryPut | auth", () => {
  test("success | correct auth", async () => {
    ...
    const result = await TestModel.queryPut(url, data);
  ...
  test("error | incorrect auth", async () => {
    ...
    await expect(TestModel.queryPut(url, data)).rejects.toThrow(
      "Request failed with status code 401"
    );
    await expect(TestModel.queryPut(url, data)).rejects.toThrow(AxiosError);
  });
});

describe(".queryPut | success", () => {
