...
describe(".queryDelete | auth", () => {
  test("success | correct auth", async () => {
    ...
    mock
      .onDelete(`${BASE_URL}/${TABLE_NAME}/${url}`)
      .reply((config) => {
    ...
    const result = await TestModel.queryDelete(url, data);
  ...
  test("error | incorrect auth", async () => {
    ...
    await expect(TestModel.queryDelete(url, data)).rejects.toThrow(
      "Request failed with status code 401"
    );
    await expect(TestModel.queryDelete(url, data)).rejects.toThrow(AxiosError);
  });
});

describe(".queryDelete | success", () => {
